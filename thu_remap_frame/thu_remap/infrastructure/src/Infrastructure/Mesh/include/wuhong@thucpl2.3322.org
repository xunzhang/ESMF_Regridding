...
public:
  ...
  Patch(const int &npnts, const double* coords_lon, const double* coords_lat) : size(npnts) {
    point tmp;
    for(int i = 0; i < npnts; ++i) {
      tmp.lon = coords_lon[i];
      tmp.lat = coords_lat[i];
...
public: 
  ...
  PatchRect(const int &npnts, const double* coords_lon, const double* coords_lat) : Patch(npnts, coords_lon, coords_lat) { compact_by_sort(); }
