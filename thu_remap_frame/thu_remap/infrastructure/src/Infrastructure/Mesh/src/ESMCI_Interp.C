...
/*edited by zhucl*/
#include <Mesh/include/ESMCI_Esmf2Thuremap.h>
	...
	void mat_patch_serial_transfer(MEField<> &src_coord_field, MEField<> &_sfield, _field &_dfield, SearchResult &sres,  Mesh &srcmesh, IWeights &iw) {
		...
		void mat_point_serial_transfer(MEField<> &sfield, _field &dfield, SearchResult &sres, IWeights &iw) {
			...
			for (; sb != se; sb++) {
				...
				/* !! I donot know the in? out? of the paras... */
				mev.Setup(elem, MEV::update_sf, &pintg);
				mev.ReInit(elem);

				std::vector<fad_type> ires(npts*dfield.dim());
				//std::cout << "before cal sens=" << ires[0] << std::endl;

				/* !! Using sF to init ires data-structure. */
				mev.GetFunctionValues(sF, &ires[0]);

				//    std::cout << "after cal sens=" << ires[0] << std::endl;
				//    std::cout << "after cal sens=" << ires[0].val() << std::endl;
				...
				for (UInt n = 0; n < npts; n++) {
					...
					for (UInt d = 0; d < 1; d++) {
						...
						/* !! I donot know the dfield.data?? */
						((double*)dfield.data(node))[d] = ires[n*dfield.dim()+d].val();

						/* !! calculate the row data-structure using node & elem <--- sres? or sfield?	
						   ^				
						   |				
						   sres				*/
						...
						//	std::cout << "npts is " << npts << "n is " << n << std::endl;
						...
						/* edited by wuhong */
						/*	std::cout << "dfield.dim() is " << dfield.dim() << std::endl;
							for(std::vector<IWeights::Entry>::iterator iter = col.begin(); iter != col.end(); ++iter)
							std::cout << "before cal? the col info is : " << *iter << std::endl;*/
						/* edited by wuhong */

						/* !! calculate the col data-structure using addc <--- col & sens					
						   ^
						   |
						   ires <--- mev & sF <--- sres or sfield
						   ^			
						   |		
						   sres		*/
						sF.dof_iterator(addc);
						...
						/* edited by wuhong */
						/*	for(std::vector<IWeights::Entry>::iterator iter = col.begin(); iter != col.end(); ++iter)
							std::cout << "Now cal? the col info is : " << *iter << std::endl;
		...
		/* edited by wuhong */
		static double cal_lats(double co[3]) {
			return atan(co[2] / sqrt(co[0] * co[0] + co[1] * co[1]));	
		}

		static double cal_lons(double co[3]) {
			if(co[1] == 0)
				return 0;
			if((co[0] == 0) && (co[1] > 0))
				return PI/2;
		...
		void Interp::mat_transfer_serial(int fpair_num, IWeights &iw, IWeights &src_frac) {
			...
			if (fpair.idata == INTERP_STD) {
				/*edited by zhucl*/
					...
					int npnts_src = srcmesh.num_nodes();
					int npnts_dst = dstmesh.num_nodes();

					printf("npnts_dst: %d\n", npnts_dst);
					printf("npnts_src: %d\n", npnts_src);
					int nlons_src, nlons_dst;
					int nlats_src, nlats_dst;
					double *center_lat_coords_src, *center_lon_coords_src;
					double *center_lat_coords_dst, *center_lon_coords_dst;
					...
					for(int k = 0; sb_src != se_src; ++sb_src, ++k) {
		...
		//			if (Par::Rank() == 0)
						for (int i = 0; i < npnts_src; i++)
							printf("%f %f\n", center_lat_coords_src[i], center_lon_coords_src[i]);
					...
					for ( ; sb != se; sb++) {
					   Search_result &sr = **sb;
						_field *dcptr = dstmesh.Getfield("coordinates_1");
						_field &dstcoord_field = *dcptr;
						
						for (int i = 0; i < sr.nodes.size(); i++) {
							double co[3];
							const MeshObj &node = *sr.nodes[i].node;
							double *c1 = dstcoord_field.data(node);
							for (int j = 0; j < 3; j++)
								co[j] = c1[j];
					...
					//printf("fuck sadgadassssssdasg\n");
//					if (Par::Rank() == 0)
						for (int i = 0; i < k; i++)
							printf("%f %f\n", center_lat_coords_dst[i], center_lon_coords_dst[i]);
				...
				/*edited by zhucl*/
				printf("sdgadgasd\n");
				mat_point_serial_transfer(*fpair.first, *fpair.second->GetNodalfield(), sres, iw);
				printf("dddaaa\n");
			}
				...
				if (fpairs[fpair_num].idata == INTERP_STD) {
#if 1
					// new interpolation
					Mesh &srcmesh = grend.GetSrcRend();
					Mesh &dstmesh = grend.GetDstRend();
					...
					for ( ; sb != se; sb++) {
					   Search_result &sr = **sb;
						...
						_field *dmptr = dstmesh.Getfield("mask_1");
						...
						for (int i = 0; i < sr.nodes.size(); i++) {
							...
							double *m = dstmask_field.data(node);
							if (*m > 0.5)
								mask_dst[k] = false;
							else
								mask_dst[k] = true;
					...
					//printf("fuck sadgadassssssdasg\n");
					if (Par::Rank() == 0)
						for (int i = 0; i < npnts_dst; i++)
							printf("%f %f\n", center_lat_coords_dst[i], center_lon_coords_dst[i]);

					ESMCI::Patch *patch_src = new ESMCI::PatchRect(npnts_src, center_lon_coords_src, center_lat_coords_src, mask_src);
					printf("src is right\n");
					MPI_Barrier(MPI_COMM_WORLD);
					
					ESMCI::Patch *patch_dst = new ESMCI::PatchRect(npnts_dst, center_lon_coords_dst, center_lat_coords_dst, mask_dst);


					patch_src->reorder(center_lon_coords_src, center_lat_coords_src);
					printf("sunxinhuatamei\n");
					//patch_src->get_reorder_mask(mask_src);
					printf("inmeedia\n");


					patch_dst->reorder(center_lon_coords_dst, center_lat_coords_dst);
					printf("bbbbbba\n");
					//patch_dst->get_reorder_mask(mask_dst);
					printf("patch reorder\n");

					nlats_src = patch_src->get_lat_size();
					nlons_src = patch_src->get_lon_size();
					nlats_dst = patch_dst->get_lat_size();
					nlons_dst = patch_dst->get_lon_size();

					grid *src_grid, *dst_grid;

					printf("Interp.C src %d, %d, %d, Rank %d\n", npnts_src, nlats_src, nlons_src, Par::Rank());
					printf("Interp.C dst %d, %d, %d\n", npnts_dst, nlats_dst, nlons_dst);

/*					printf("Interp.C src coord\n");
					for (int i = 0; i < npnts_src; i++)
						printf("%f %f\n", center_lat_coords_src[i], center_lon_coords_src[i]);
					printf("\nInterp.C dst coord\n");
					for (int i = 0; i < npnts_dst; i++)
					...
					printf("\n");
*/
					src_grid = new grid(npnts_src, nlats_src, nlons_src, center_lat_coords_src, center_lon_coords_src, mask_src);
					printf("new grid2\n");
					dst_grid = new grid(npnts_dst, nlats_dst, nlons_dst, center_lat_coords_dst, center_lon_coords_dst, mask_dst);
					printf("new grid3\n");
...
//					MEField<> *dst_mask_ptr = dstmesh.GetField("mask");
					...
					for (int k = 0; sb_dst != se_dst; ++sb_dst, ++k) {
...
*/
					/* edited by wuhong */
					/*
					   std::cout << "Hello wuhong" << std::endl;	
					   Mesh &srcmesh = grend.GetSrcRend();
					   Mesh &dstmesh = grend.GetDstRend();
					   ...
					   center_lat_coords_src = new double [npnts_src];
					   center_lon_coords_src = new double [npnts_src];
					   aaaa = new double[npnts_src];
					   bbbb = new double[npnts_src];
					   mask_src = new bool [npnts_src];
					   MEField<> *src_field = srcmesh.GetCoordField();
					   MEField<> *src_mask_ptr = srcmesh.GetField("mask");
					   ...
					   if (Par::Rank() == 0) {
					   std::cout << "npnts_src  " << npnts_src << std::endl;

					   ESMCI::Patch rect_patch(npnts_src, center_lon_coords_src, center_lat_coords_src);
					   rect_patch.reorder(aaaa, bbbb);
					   */    
					/* check the coord info below */
					/*		  for (int i = 0; i < npnts_src; i++) 
							  printf("%f %f\n", aaaa[i], bbbb[i]);
							  std::cout << std::endl;
							  for (int i = 0; i < npnts_src; i++) 
							  std::cout << center_lat_coords_src[i] << " " << center_lon_coords_src[i] << std::endl;
					//		  std::cout << "----------------------------" << std::endl;
					}
					...
					*/
					...
					/* edited by zhucl */
					   ...
					   Search_result &sr = **sb;
					   int num_src_nodes;
					   double src_coords[120];
					   get_elem_coords(sr.elem, src_field, 3, 120, &num_src_nodes, src_coords);
					   if (Par::Rank() == 0) {
									std::cout << "src Elem" << std::endl;
					for (int i = 0; i < num_src_nodes; i++) {
					double co[3];
					co[0] = src_coords[3*i];
					co[1] = src_coords[3*i + 1];
					co[2] = src_coords[3*i + 2];
					double lat = 180 * cal_lats(co) / PI;
					double lon = 180 * cal_lons(co) / PI;
					std::cout << lat << " " << lon << std::endl;
					}
									std::cout << "dst node" << std::endl;
					_field *dcptr = dstmesh.Getfield("coordinates_1");
					_field &dstcoord_field = *dcptr;
					for (int i = 0; i < sr.nodes.size(); i++) {
					double co[3];
...
//				    mat_point_serial_transfer(*sFR, *dfR, sres, iw);
					printf("sdgadsgadsgashksdljglahdsalf\n");
				}
