...
void bilinear_remap_2D_operator::init_remap()
{
	...
	src_grid_mask = new int[npts_src];
	dst_grid_mask = new int[npts_dst];
	for(i = 0; i < npts_dst; i ++)
		if(mask_dst[i]) dst_grid_mask[i] = 1; else dst_grid_mask[i] = 0;
	
	for(i = 0; i < npts_src; i ++)
		if(mask_src[i]) src_grid_mask[i] = 1; else src_grid_mask[i] = 0;
    ...
    nlats_src = grid_src_local->get_num_lats();
    nlons_dst = grid_dst_local->get_num_lons();
    nlats_dst = grid_dst_local->get_num_lats();
	...
	if((retval = nc_def_dim(ncid, "nlons_src", nlons_src, &nc_nlons_src_id)))
		ERR(retval);
	if((retval = nc_def_dim(ncid, "nlats_src", nlats_src, &nc_nlats_src_id)))
		ERR(retval);
	if((retval = nc_def_dim(ncid, "nlons_dst", nlons_dst, &nc_nlons_dst_id)))
		ERR(retval);
	if((retval = nc_def_dim(ncid, "nlats_dst", nlats_dst, &nc_nlats_dst_id)))
		ERR(retval);
	...
	if((retval = nc_def_var(ncid, "src_mask", NC_INT, 1, &nc_src_grid_size_id, &nc_src_mask_id)))
		ERR(retval);
	...
	if((retval = nc_put_var_double(ncid, nc_remap_matrix_id, wgt_values)))
		ERR(retval);
	if((retval = nc_put_var_int(ncid, nc_src_mask_id, src_grid_mask)))
		ERR(retval);
	if((retval = nc_put_var_int(ncid, nc_dst_mask_id, dst_grid_mask)))
		ERR(retval);	
